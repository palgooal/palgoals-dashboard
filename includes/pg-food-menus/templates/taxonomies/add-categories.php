    ...
    <div class="offcanvas-body customer-body">
        <div class="card-body">
            <form id="add-category-form">
                <!-- Name -->
                <div class="mb-3">
                    <label for="category-name"><?php esc_html_e('Name', 'palgoals-dash'); ?></label>
                    <input type="text" id="category-name" name="name" class="form-control" required>
                </div>
                <!-- Slug -->
                <div class="mb-3">
                    <label for="category-slug"><?php esc_html_e('Slug', 'palgoals-dash'); ?></label>
                    <input type="text" id="category-slug" name="slug" class="form-control">
                </div>
                <!-- Parent -->
                <div class="mb-3">
                    <label for="parent-category"><?php esc_html_e('Parent Category', 'palgoals-dash'); ?></label>
                ...
                <!-- Submit -->
                <button type="submit" class="btn btn-primary"><?php esc_html_e('Add Category', 'palgoals-dash'); ?></button>
            </form>
        </div>
    </div>
</div>
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($name) {
        ...
        if (!is_wp_error($term)) {
            ...
            echo '<p>' . esc_html__('Category Added Successfully:', 'palgoals-dash') . ' ' . esc_html($term['term_id']) . '</p>';
        } else {
            echo '<p>' . esc_html__('Error:', 'palgoals-dash') . ' ' . esc_html($term->get_error_message()) . '</p>';
        }
    } else {
        echo '<p>' . esc_html__('No Name Provided', 'palgoals-dash') . '</p>';
    }
}
?>
